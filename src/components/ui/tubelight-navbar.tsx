...
export function NavBar({ items, className }: NavBarProps) {
  ...
  // Derive the active tab based on the current location path
        ...
        {items.map((item) => {
          ...
          return (
            <Link
              ...
              to={item.url}
              className={cn(
                "relative cursor-pointer text-sm font-semibold px-3 sm:px-6 py-2 rounded-full transition-colors pointer-events-auto",
            ...
            >
              ...
              {isActive && (
                <motion.div
                  layoutId="lamp"
                  className="absolute inset-0 w-full bg-navbar-selection rounded-full -z-10"
                  initial={false}
                  transition={{
                    type: "spring",
                    stiffness: 300,
                ...
                >
                  <div className="absolute -top-2 left-1/2 -translate-x-1/2 w-8 h-1 bg-primary rounded-t-full">
                    <div className="absolute w-12 h-6 bg-primary/35 rounded-full blur-md -top-2 -left-2" />
